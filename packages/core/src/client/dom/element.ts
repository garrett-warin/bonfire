...
function collectAttributeMap(
	...
	for (const attrName of attrNames) {
		if (String(attrName).startsWith("runtimekit-attr")) continue;
...
export default function (client: RuntimeKitClient, self: typeof window) {
	const attrObject = {
	...
	client.Proxy("Element.prototype.getAttribute", {
		apply(ctx) {
			...
			if (name.startsWith("runtimekit-attr")) {
			...
			if (
				client.natives.call(
					"Element.prototype.hasAttribute",
					ctx.this,
					`runtimekit-attr-${name}`
				)
			) {
				const attrib = ctx.fn.call(ctx.this, `runtimekit-attr-${name}`);
	...
	client.Proxy("Element.prototype.getAttributeNames", {
		apply(ctx) {
			...
			const cleaned = attrNames.filter(
				(attr) => !attr.startsWith("runtimekit-attr")
			);
	...
	client.Proxy("Element.prototype.getAttributeNode", {
		apply(ctx) {
			if (String(ctx.args[0]).startsWith("runtimekit-attr"))
				return ctx.return(null);
		},
	});
		...
		apply(ctx) {
			...
			if (ruleList) {
				...
				if (ret == null) {
					...
					ctx.fn.call(ctx.this, `runtimekit-attr-${name}`, value);
					ctx.return(undefined);

					return;
				}
				ctx.args[1] = ret;
				ctx.fn.call(ctx.this, `runtimekit-attr-${ctx.args[0]}`, value);
			}
		},
	});
	...
	client.Proxy("Element.prototype.removeAttribute", {
		apply(ctx) {
			...
			if (name.startsWith("runtimekit-attr")) return ctx.return(undefined);
			if (
				client.natives.call("Element.prototype.hasAttribute", ctx.this, name)
			) {
				ctx.fn.call(ctx.this, `runtimekit-attr-${ctx.args[0]}`);
			}
		},
	});
	...
	client.Proxy("Element.prototype.toggleAttribute", {
		apply(ctx) {
			const name = String(ctx.args[0]);
			if (name.startsWith("runtimekit-attr")) return ctx.return(false);
			if (
				client.natives.call("Element.prototype.hasAttribute", ctx.this, name)
			) {
				ctx.fn.call(ctx.this, `runtimekit-attr-${ctx.args[0]}`);
		...
		set(ctx, value: string) {
			...
			if (
				...
				client.natives.call(
					"Element.prototype.setAttribute",
					ctx.this,
					"runtimekit-attr-script-source-src",
	...
	client.Trap(
		...
		],
		{
			get(ctx) {
				...
				try {
					if (!(RUNTIMEKITCLIENT in realwin)) {
	...
	client.Proxy(
		...
		],
		{
			apply(ctx) {
				const doc = ctx.call();
				if (doc) {
					// we trap the contentDocument, this is really the runtimekit version
