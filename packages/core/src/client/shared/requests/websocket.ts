import { type BareCompatibleWebSocket as ChannelCompatibleWebSocket } from "@mercuryworkshop/proxy-transports";
...
type FakeWebSocketState = {
	...
	url: string;
	binaryType: string;
	barews: ChannelCompatibleWebSocket;
...
export default function (client: RuntimeKitClient, self: GlobalThis) {
	...
	client.Proxy("WebSocket", {
		construct(ctx) {
			...
			const barews = client.connector.createWebSocket(url, ctx.args[1], [
	...
	client.Proxy("WebSocketStream", {
		construct(ctx) {
			...
			const barews = client.connector.createWebSocket(ctx.args[0], ctx.args[1], [
				["User-Agent", self.navigator.userAgent],
				["Origin", client.url.origin],
			]);
