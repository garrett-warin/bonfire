...
import { RuntimeKitContext } from "@/shared";
...
export type HtmlContext = {
	// should we inject runtimekit scripts at the top of the document?
	loadScripts: boolean;
...
export class IncrementalHtmlRewriter {
	...
	constructor(
		private readonly context: RuntimeKitContext,
...
function rewriteHtmlInner(
	...
	if (htmlcontext.loadScripts) {
		const script = (src: string) =>
			new Element("script", { src, "runtimekit-injected": "true" });
	...
	function traverse(node: ChildNode) {
		if ("attribs" in node) {
			for (const key in node.attribs) {
				if (key == "runtimekit-attr-script-source-src") {
					if (node.children[0] && "data" in node.children[0])
						node.children[0].data = atob(node.attribs[key]);
					continue;
				}

				if (key.startsWith("runtimekit-attr-")) {
					node.attribs[key.slice("runtimekit-attr-".length)] = node.attribs[key];
...
function traverseParsedHtml(
			...
			for (const attr in rule) {
				...
				if (sel === "*" || sel.includes(node.name)) {
					if (node.attribs[attr] !== undefined) {
						...
						node.attribs[`runtimekit-attr-${attr}`] = value;
					}
				}
			}
		}
	...
	if (
		...
		if (isScriptType(scriptBlockType)) {
			...
			node.attribs["runtimekit-attr-script-source-src"] = bytesToBase64(
