...
if [ -f out/.build-hash ] && [ -f ../../dist/runtimekit.wasm ] && [ "$SRC_HASH" != "unknown" ] && grep -q "$SRC_HASH" out/.build-hash; then
...
cp rewriter/wasm/out/optimized.wasm dist/runtimekit.wasm
