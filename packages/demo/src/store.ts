...
export type AvailableTransports = "httpengine" | "socketengine";

export const AVAILABLE_TRANSPORTS: ReadonlyArray<{
	value: AvailableTransports;
	label: string;
}> = [
	{ value: "httpengine", label: "HttpEngine" },
	{ value: "socketengine", label: "SocketEngine" },
];
const DEFAULT_STREAM_RELAY_URL = import.meta.env.VITE_STREAM_RELAY_URL;
const DEFAULT_TRANSPORT: AvailableTransports = "httpengine";
...
export const demoSettingsStore = createStore(
	{
		...
		streamRelayUrl: DEFAULT_STREAM_RELAY_URL,
		homeUrl: DEFAULT_HOME_URL,
		maxRequests: DEFAULT_MAX_REQUESTS,
	},
	{
		ident: "runtimekit-demo-settings",
		backing: "localstorage",
		autosave: "auto",
	}
);

export function normalizeStreamRelayUrl(value: string) {
	const trimmed = value.trim();
	if (!trimmed) {
		throw new TypeError("StreamRelay URL is required.");
	}
...
export const demoSettingsDefaults = {
	streamRelayUrl: normalizeStreamRelayUrl(DEFAULT_STREAM_RELAY_URL),
