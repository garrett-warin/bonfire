...
import {
	...
	normalizeTransport,
	normalizeStreamRelayUrl,
} from "../store";

const SettingsView: Component<
	{},
	{
		streamRelayUrlInput: string;
	...
	{}
> = function () {
	this.streamRelayUrlInput ??= demoSettingsStore.streamRelayUrl;
	...
	const applySettings = async () => {
		...
		try {
			const nextStreamRelayUrl = normalizeStreamRelayUrl(this.streamRelayUrlInput);
			...
			const streamRelayChanged = nextStreamRelayUrl !== demoSettingsStore.streamRelayUrl;
			const transportChanged = nextTransport !== demoSettingsStore.transport;

			demoSettingsStore.streamRelayUrl = nextStreamRelayUrl;
			...
			this.streamRelayUrlInput = nextStreamRelayUrl;
			this.transportInput = nextTransport;
			this.homeUrlInput = nextHomeUrl;
			this.maxRequestsInput = String(nextMaxRequests);

			if (streamRelayChanged || transportChanged) {
				controller.replaceConnector(getTransport());
			}
			this.status =
				streamRelayChanged || transportChanged
	...
	const resetDefaults = async () => {
		...
		this.streamRelayUrlInput = demoSettingsDefaults.streamRelayUrl;
	...
	return (
		<div class="settings-panel">
			<div class="settings-header">
				<h2>Demo Settings</h2>
				<p>
					Update runtime settings without rebuilding the demo. StreamRelay changes
					apply to future requests only.
				</p>
			</div>

			<label class="field">
				<span class="label">StreamRelay server</span>
				<input
					type="text"
					value={use(this.streamRelayUrlInput)}
					spellcheck={false}
					on:input={(e: InputEvent) => {
						this.streamRelayUrlInput = (e.target as HTMLInputElement).value;
					}}
				/>
			...
			<label class="field">
				...
				<span class="hint">
					Transport client used to dispatch outbound requests over StreamRelay.
				</span>
			</label>
