// HTTP cache plugin for RuntimeKitFetchHandler.
//
...
// resources. This plugin caches the **upstream** response (the ChannelResponse
...
import {
	ChannelResponse,
	type RuntimeKitFetchRequest,
	type RuntimeKitHeaders,
} from "@mercuryworkshop/runtimekit";
import { ManagedPlugin } from "@mercuryworkshop/runtimekit-controller";
import type { Frame } from "@mercuryworkshop/runtimekit-controller";

export const CACHE_NAME = "runtimekit-http-cache-v2";
 ...
 * headers, not yet through runtimekit's response-header rewriter.
 */
...
/** Rebuild a Headers object from the ChannelResponse's rawHeaders array. */
...
/**
 * Turn an upstream ChannelResponse into a ChannelResponse that:
 ...
 */
async function rebuildChannelResponseWithBuffer(
	upstream: ChannelResponse
): Promise<{ replacement: ChannelResponse; bodyBuffer: ArrayBuffer | null }> {
	const status = upstream.status;
	const isNullBody = NULL_BODY_STATUSES.has(status);

	const headers = nativeHeadersFromRaw(upstream.rawHeaders);

	if (isNullBody) {
		return {
			replacement: ChannelResponse.fromNativeResponse(
				new Response(null, {
					status,
...
/**
 * RFC-9111-ish HTTP cache for RuntimeKitFetchHandler.
 *
...
export class HttpCachePlugin extends ManagedPlugin {
	...
	private cameFromCache = new WeakMap<RuntimeKitFetchRequest, true>();

	constructor(options: HttpCachePluginOptions = {}) {
		super("runtimekit-http-cache", []);
	...
	install(frame: Frame): void {
		...
		this.tap(hooks.request, async (ctx, props) => {
			...
			// Build a ChannelResponse around the stored bytes/headers and hand
			...
			const earlyResponse = ChannelResponse.fromNativeResponse(
		...
		this.tap(hooks.preresponse, async (ctx, props) => {
			...
			// Drain the stream once and rebuild the ChannelResponse around the
			// buffered copy so the rest of doHandleFetch can still read it.
			const { replacement, bodyBuffer } = await rebuildChannelResponseWithBuffer(
				props.response
			);
			...
			try {
				...
				console.warn("[runtimekit-http-cache] cache.put failed:", err);
			}
		});
	}
	...
	async bust(): Promise<boolean> {
		try {
			...
			console.error("[runtimekit-http-cache] bust failed:", err);
			return false;
		}
	}
}
