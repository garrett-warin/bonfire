...
// is redirected through the $runtimekit__<name> accessor pair installed on
...
export default [
	...
	basicTest({
		...
		js: `
			// the $runtimekit__location getter reads this.location, so a page-level
	...
	// ------------------------------------------------------------------
	basicTest({
		// KNOWN FAILURE: the trap sees $runtimekit__location instead of location,
	...
	basicTest({
		...
		// property literally called $runtimekit__location. Null-prototype
	...
	basicTest({
		// KNOWN FAILURE: rewritten to `delete o.$runtimekit__location`, which
	...
	basicTest({
		// KNOWN FAILURE: `super.location` becomes `super.$runtimekit__location`,
