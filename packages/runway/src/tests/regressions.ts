...
export default [
	...
	// runtimekit/html
	// module script metadata must stay in runtimekit's reserved query params.
			...
			server.on("request", (req, res) => {
				...
				if (req.url?.startsWith("/mod.js")) {
					...
					if (url.searchParams.get("real") !== "1") {
						...
						res.end(
							`window.fail("runtimekit module marker leaked upstream", { search: ${JSON.stringify(url.search)} })`
						);
					} else {
						res.end(`
							import "#dep";
				...
				if (req.url?.startsWith("/dep.js")) {
					...
					if (url.searchParams.get("real") !== "1") {
						...
						res.end(
							`window.fail("runtimekit module marker leaked upstream from dependency", { search: ${JSON.stringify(url.search)} })`
						);
					} else {
	...
	// runtimekit/core/rewriter
	// non-computed keys of a destructured objects were wrapped with $runtimekit$prop, causing invalid syntax
