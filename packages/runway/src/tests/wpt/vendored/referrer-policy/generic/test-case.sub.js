...
const referrerUrlResolver = {
	...
	origin: function (sourceUrl) {
		return [stripUrlForUseAsReferrer(sourceUrl, true), undefined];
	},
	"stripped-referrer": function (sourceUrl) {
		return [
			stripUrlForUseAsReferrer(sourceUrl, false),
...
function checkResult(scenario, expectation, result) {
	...
	const possibleReferrerUrls = referrerUrlResolver[expectation](referrerSource);

	// Check the reported URL.
	assert_in_array(result.referrer, possibleReferrerUrls, "document.referrer");
	assert_in_array(
...
function runLengthTest(scenario, urlLength, expectation, testDescription) {
	...
	promise_test((t) => {
		...
		return invokeScenario(scenario).then((result) =>
			checkResult(scenario, expectation, result)
		);
	}, testDescription);
}
...
function TestCase(scenarios, sanityChecker) {
	function shouldSkipScenario(scenario) {
		// RuntimeKit currently treats pages as secure contexts, so the imported WPT
		// cases that rely on explicit HTTPS destinations or downgrade behavior
		// don't produce meaningful results yet.
		return (
			scenario.origin === "same-https" ||
			scenario.origin === "cross-https" ||
			scenario.redirection === "downgrade"
		);
	}
	...
	function runTest(scenario) {
		...
		promise_test((_) => {
			return invokeScenario(scenario).then((result) =>
				checkResult(scenario, scenario.expectation, result)
			);
	...
	function runTests() {
		for (const scenario of scenarios) {
			if (shouldSkipScenario(scenario)) continue;
			runTest(scenario);
		}
	}
