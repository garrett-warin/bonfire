...
//                 Anything without checkglobal() runs in both the runtimekit and
...
export default [
	...
	basicTest({
		...
		js: `
			...
			// (wrapfn, the $runtimekit__eval accessor, wrappropertyfn, and a
	...
	basicTest({
		...
		// leaves it outside the argument list: `eval $runtimekit$rewrite(("x"))`.
	...
	basicTest({
		...
		// lands *inside* the comment: `eval/$runtimekit$rewrite(*c*/("x"))`.
	...
	basicTest({
		// KNOWN FAILURE: Reflect.get bypasses the $runtimekit__eval accessor and
	...
	basicTest({
		...
		js: `
			...
			const s = f.toString();
			assert(!s.includes("runtimekit"), "eval'd function source must not leak rewriter internals: " + s);
	...
	// $runtimekit__eval accessor on Object.prototype - including accesses on
	...
	basicTest({
		...
		// `delete o.$runtimekit__eval`, which deletes nothing and reports success.
	...
	basicTest({
		...
		// `eval($runtimekit$rewrite(...))` without checking whether `eval` still
