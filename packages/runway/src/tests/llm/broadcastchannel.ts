...
// {$runtimekit$messagetype, $runtimekit$origin, $runtimekit$data} envelope.
...
export default [
	basicTest({
		...
		js: `
			const channel = "runtimekit-bc-string-" + Date.now() + "-" + Math.random();
	...
	basicTest({
		...
		js: `
			const channel = "runtimekit-bc-object-" + Date.now() + "-" + Math.random();
			const a = new BroadcastChannel(channel);
			const b = new BroadcastChannel(channel);
			b.addEventListener("message", (event) => {
	...
	basicTest({
		...
		js: `
			// The proxy wraps data in an envelope with $runtimekit$ keys.
			// Page code must never see those keys on event.data.
			const channel = "runtimekit-bc-envelope-" + Date.now() + "-" + Math.random();
			const a = new BroadcastChannel(channel);
			const b = new BroadcastChannel(channel);
			b.addEventListener("message", (event) => {
				assertEqual(typeof event.data, "object", "data should be an object");
				assert(!("$runtimekit$data" in event.data), "$runtimekit$data should not leak");
				assert(!("$runtimekit$origin" in event.data), "$runtimekit$origin should not leak");
				assert(!("$runtimekit$messagetype" in event.data), "$runtimekit$messagetype should not leak");
	...
	basicTest({
		...
		js: `
			const channel = "runtimekit-bc-onmessage-" + Date.now() + "-" + Math.random();
			const a = new BroadcastChannel(channel);
			const b = new BroadcastChannel(channel);
			b.onmessage = (event) => {
	...
	basicTest({
		...
		js: `
			const channel = "runtimekit-bc-origin-" + Date.now() + "-" + Math.random();
			const a = new BroadcastChannel(channel);
			const b = new BroadcastChannel(channel);
			b.addEventListener("message", (event) => {
				...
				assert(!event.origin.includes("$runtimekit$"), "event.origin should not be the envelope");
				a.close();
				b.close();
				pass();
			});
	...
	basicTest({
		...
		js: `
			const channel = "runtimekit-bc-name-" + Date.now() + "-" + Math.random();
	...
	basicTest({
		...
		js: `
			...
			const channel = "runtimekit-bc-self-" + Date.now() + "-" + Math.random();
			const a = new BroadcastChannel(channel);
	...
	basicTest({
		...
		js: `
			const nameA = "runtimekit-bc-isoA-" + Date.now() + "-" + Math.random();
			const nameB = "runtimekit-bc-isoB-" + Date.now() + "-" + Math.random();
	...
	basicTest({
		...
		js: `
			const channel = "runtimekit-bc-multi-" + Date.now() + "-" + Math.random();
			const a = new BroadcastChannel(channel);
			const b = new BroadcastChannel(channel);
	...
	basicTest({
		...
		js: `
			const channel = "runtimekit-bc-closed-" + Date.now() + "-" + Math.random();
			const a = new BroadcastChannel(channel);
			const b = new BroadcastChannel(channel);
			let bGot = false;
			b.addEventListener("message", () => {
	...
	basicTest({
		...
		js: `
			const channel = "runtimekit-bc-primitives-" + Date.now() + "-" + Math.random();
			const a = new BroadcastChannel(channel);
			const b = new BroadcastChannel(channel);
	...
	basicTest({
		...
		js: `
			const channel = "runtimekit-bc-array-" + Date.now() + "-" + Math.random();
			const a = new BroadcastChannel(channel);
			const b = new BroadcastChannel(channel);
			b.addEventListener("message", (event) => {
				...
				assert(!("$runtimekit$data" in event.data), "$runtimekit$data should not appear on the array");
				a.close();
				b.close();
				pass();
			});
	...
	basicTest({
		...
		js: `
			const channel = "runtimekit-bc-nested-" + Date.now() + "-" + Math.random();
			const a = new BroadcastChannel(channel);
			const b = new BroadcastChannel(channel);
			b.addEventListener("message", (event) => {
				assertEqual(event.data.outer.inner.value, "deep", "nested value should round-trip");
				assert(!("$runtimekit$data" in event.data), "envelope must not leak at top level");
				assert(!("$runtimekit$data" in event.data.outer), "envelope must not appear nested");
				a.close();
				b.close();
				pass();
			});
	...
	basicTest({
		...
		js: `
			const channel = "runtimekit-bc-remove-" + Date.now() + "-" + Math.random();
			const a = new BroadcastChannel(channel);
			const b = new BroadcastChannel(channel);
			let calls = 0;
			const handler = () => { calls++; };
	...
	basicTest({
		...
		js: `
			const channel = "runtimekit-bc-origin-fake";
			const a = new BroadcastChannel(channel);
			const b = new BroadcastChannel(channel);
			b.addEventListener("message", (event) => {
				assertEqual(
	...
	multiFrameTest({
		...
		// BroadcastChannel is same-origin per spec. Since every runtimekit
		...
		root: {
			js: () => `
				const bc = new BroadcastChannel("runtimekit-bc-xorigin");
				let leaked = false;
	...
	multiFrameTest({
		...
		root: {
			js: () => `
				const bc = new BroadcastChannel("runtimekit-bc-mframe");
				bc.addEventListener("message", (event) => {
					assertEqual(event.data, "from-child", "parent should receive unwrapped child message");
					assert(
						typeof event.data !== "object" || !("$runtimekit$data" in event.data),
